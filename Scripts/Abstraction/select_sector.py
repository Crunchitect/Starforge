...
def venus_sector_text():
    global sector_text
    from Scripts.Proc.WorldGen.Planets.Venus.world_sphere import sector
    sector_text.text += f'\nVenus Sector {sector}' if sector != -1 else ''
...
def update(cam: Union[EditorCamera, FirstPersonController]):
    ...
    from Scripts.Proc.WorldGen.Planets.Earth.world_sphere import sector
    sector_text.text = f'Earth Sector {sector}' if sector != -1 else ''
    venus_sector_text()
    if sector == -1:
